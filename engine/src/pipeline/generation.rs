        ...
        + Sync,
{
    ...
    pub async fn generate_cards_batch(
        ...
        let raw_json = {
            ...
            .map_err(|_| {
                anyhow::anyhow!(
                    "LLM generation call timed out after {:?}",
                    self.llm_call_timeout
                )
            })??
        };
        ...
        let json_array: Vec<serde_json::Value> =
            if let Some(arr) = parsed_root.get_mut("cards").and_then(|v| v.as_array_mut()) {
                ...
                return Err(anyhow::anyhow!(
                    "LLM returned an object without 'cards' array"
                ));
            };
        ...
        let node_path_for_extraction =
            crate::skill_tree::get_node_path(tree_root, skill_node_id).unwrap_or_default();
        ...
        for value in json_array.into_iter() {
            ...
            let parsed = match self.parse_and_validate(req, &card_json_str).await {
                ...
                Err(feedback) => {
                    ...
                    match self
                        .retry_single_card(tree_root, req, skill_node_id, &feedback, &llm_semaphore)
                        .await
                    {
                        Ok(result) => result,
                        Err(e) => {
            ...
            fetch_tasks.push(self.process_single_card(
                ...
                skill_node_id,
                skill_name.clone(),
                node_for_extraction.clone(),
                node_path_for_extraction.clone(),
                llm_sem,
                pp_sem,
                req,
            ));
