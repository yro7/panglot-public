...
use crate::traits::{Language, MorphologyInfo};
...
/// typed validation. The three extension points on `Language` (`build_extraction_schema`,
/// `extra_extraction_directives`, `post_process_extraction`) should delegate here.
pub trait Agglutinative: Language
where
        ...
        + Sync,
{
    ...
    /// Default implementation uses `SchemaGenerator` with the combined helper type.
...
// Lives in `core` (not `engine`) so that `Language::build_extraction_schema()`
